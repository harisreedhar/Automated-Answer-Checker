...
class Model:
	...
	def setupCTC(self):
		...
		if self.decoderType == DecoderType.BestPath:
			...
			wordChars = open(mypath() +'/model/wordCharList.txt').read().splitlines()[0]
			corpus = open(mypath() +'/data/corpus.txt').read()
	...
	def save(self):
		"save model to file"
		self.snapID += 1
		self.saver.save(self.sess, mypath() +'/model/snapshot', global_step=self.snapID)
